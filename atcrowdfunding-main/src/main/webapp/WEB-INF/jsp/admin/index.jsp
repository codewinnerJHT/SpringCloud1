 ...
 <hr style="clear:both;">
              ...
              <c:forEach items="${page.list }" var="admin" varStatus="status">
                <tr>
                  ...
                  <td>
					  ...
					  <button  type="button" adminId="${admin.id}" class="delectBtnClass btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>
				  </td>
                </tr>
                </c:forEach>
              </tbody>
			  <tfoot>
			     <tr >
				     <td colspan="6" align="center">
						...
						<c:if test="${num!=page.pageNum }">
							<li><a href="${PATH }/admin/index?condition=${param.condition }&pageNum=${num }">${num } <span class="sr-only">(current)</span></a></li>
						</c:if>
						</c:forEach>
     ...
     <%@ include file="/WEB-INF/jsp/common/js.jsp" %>
        <script type="text/javascript">
            ...
            $(".delectBtnClass").click(function(){
            	var id=$(this).attr("adminId")
            	layer.confirm('您是否确认删除该条数据？',{btn:["确定","取消"]},function(index){
            		window.location.href='${PATH}/admin/dodelect?pageNum=${page.pageNum }&id='+id;
            		layer.close(index)
            	},function(index){
            ...
            $("#deleteBatchBtn").click(function(){
            	...
            	ids=array.join(",");
            	layer.confirm("您确定删除这些数据吗？",{btn:['确定','取消']},function(index){
